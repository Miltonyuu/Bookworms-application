...
var microbundle = function (options) {
  try {
    function _temp10() {
      function _temp8(_isDir2) {
        function _temp7(_jsOrTs) {
          function _temp6(_jsOrTs2) {
            function _temp5(_isDir3) {
              if (_isDir3) {
                main = path.resolve(main, ((removeScope(options.pkg.name)) + ".js"));
              }

              options.output = main;
              return Promise.resolve(asyncro.map([].concat(options.input), function (file) {
                try {
                  file = path.resolve(cwd, file);
                  return Promise.resolve(isDir(file)).then(function (_isDir) {
                    if (_isDir) {
            ...
            _concat.call([], _temp3 ? _jsOrTs2 : _jsOrTs2 || options.pkg.module).map(function (file) { return glob(file); }).forEach(function (file) {
              var ref;

              return (ref = options.input).push.apply(ref, file);
            });

            var main = path.resolve(cwd, options.output || options.pkg.main || 'dist');

            var _temp4 = !main.match(/\.[a-z]+$/);

            return _temp4 ? _temp5(_temp4) : Promise.resolve(isDir(main)).then(_temp5);
          }

          return _temp3 || _temp2 || _jsOrTs ? _temp6(_temp3 ? _jsOrTs : _temp2 || _jsOrTs || jsOrTs('index')) : Promise.resolve(_temp3 ? _jsOrTs : _temp2 || _jsOrTs || jsOrTs('index')).then(_temp6);
        }

        return _temp3 || _temp2 || !_isDir2 ? _temp7(_temp3 ? _isDir2 : _temp2 || _isDir2 && jsOrTs('src/index')) : Promise.resolve(_temp3 ? _isDir2 : _temp2 || _isDir2 && jsOrTs('src/index')).then(_temp7);
      }

      if (!options.pkg.name) {
        options.pkg.name = path.basename(options.cwd);
      ...
      var _concat = [].concat,
            _temp3 = options.entries && options.entries.length,
            _temp2 = _temp3 || options.pkg.source && path.resolve(cwd, options.pkg.source);

      return _temp3 || _temp2 ? _temp8(_temp3 ? options.entries : _temp2 || isDir(path.resolve(cwd, 'src'))) : Promise.resolve(_temp3 ? options.entries : _temp2 || isDir(path.resolve(cwd, 'src'))).then(_temp8);
    }
    ...
    var cwd = options.cwd = path.resolve(process.cwd(), options.cwd),
        hasPackageJson = true;

    var _temp9 = _catch(function () {
      var _JSON = JSON,
            _parse = _JSON.parse;
      return Promise.resolve(readFile(path.resolve(cwd, 'package.json'), 'utf8')).then(function (_readFile) {
        options.pkg = _parse.call(_JSON, _readFile);
      });
    }, function (err) {
      stderr(chalk.yellow(((chalk.yellow.inverse('WARN')) + " no package.json found. Assuming a pkg.name of \"" + (path.basename(options.cwd)) + "\".")));
...
function createConfig(options, entry, format, writeMeta) {
  ...
  var aliases = {}; // since we transform src/index.js, we need to rename imports for it:

  if (options.multipleEntries) {
    aliases['.'] = './' + path.basename(options.output);
  }
  ...
  var nameCache = {};
  var mangleOptions = options.pkg.mangle || false;
  ...
  function loadNameCache() {
    try {
      nameCache = JSON.parse(fs.readFileSync(path.resolve(options.cwd, 'mangle.json'), 'utf8'));
    } catch (e) {}
  }
